function [val, rate] = smcKS34461A_Ramp(ico, val, rate)
...
switch ico(2) % channel
    case 1
        switch ico(3)
            case 0
                ...
                % reading. Set sampling time to immediate (using NPLC), and sampel
                % count to 1.
                fprintf(smdata.inst(ico(1)).data.inst, 'ABORt');
                fprintf(smdata.inst(ico(1)).data.inst, ':TRIG:SOURce IMM;:TRIG:COUNt 1;:SAMP:COUNt 1');
	...
	case {3, 4, 5, 6, 7}
        switch ico(3)
            case 0
                % this blocks until all values are available
                % First interplote the data to integer times of npoints using nearest method.
                npts = smdata.inst(ico(1)).datadim(ico(2));
                myval = sscanf(query(smdata.inst(ico(1)).data.inst,  'FETCH?'), '%f,')';
                interpPoints = ceil(length(myval)./npts);
                interpVals = interp1(1:length(myval),myval,linspace(1,length(myval),interpPoints*npts),"nearest");
                % Then take average of interploted data as output.
                avgVals = movmean(interpVals,[interpPoints-1 0]);
                val = avgVals(interpPoints:interpPoints:end);
            case 3
                % trigger DMM
                % trigger(smdata.inst(ico(1)).data.inst);
                fprintf(smdata.inst(ico(1)).data.inst,'*TRG');
            ...
            case 5
                ...
                % K34461A can't use sample timer to set interval between measurements.
                % The time between the trigger and the first measurement is set by TRIG:DEL.
                ...
                switch ico(2)
                    ...
                    case 7 % Buffered acquisition with 0.02 NPLC
	...
	case 8 % voltage measurement range (needed when autorange turned off)
        switch ico(3)
            case 0
                % this blocks until all values are available
