    ...
    case 1 %Read Voltage
        switch ic(3); %Operation: 0 for read, 1 for write
             ...
             case 0 %just read voltage for now
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENS:VOLT:DC:DIGits 3.5'));
                ...
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('VOLTage:DC:NPLCycles 0.01'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('VOLTage:DC:RANGe:AUTO OFF'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('VOLTage:DC:AVER:STAT OFF'));
                ...
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENS:FUNC "VOLT:DC"'));
                val_char=query(smdata.inst(ic(1)).data.inst, ':data?');
%                 val=bin2dec(valbin)
%                 fprintf(smdata.inst(ic(1)).data.inst, sprintf(':data?'));
%                 val=fread(smdata.inst(ic(1)).data.inst,1,'single')
...
%                 fprintf(smdata.inst(ic(1)).data.inst,'TRIG:SOUR EXT'); 
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('FORMAT:DATA ASCII'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('FORMAT:ELEM READ'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('SYSTem:AZERo:STATe OFF'));%set up conditions for maximum measurement speed
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('VOLTage:DC:NPLCycles 0.01'));
...
%                 v_size = size(strfind(dmm_data,'VDC'),2);
%                 vstart = [[1],(strfind(dmm_data,'#,')+2)];
%                 vend = strfind(dmm_data,'VDC')-1;
%                 tstart=strfind(dmm_data,'VDC,')+5;
...
%                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('ROUTE:SCAN:LSEL INT'));
 ...
 %                fprintf(smdata.inst(ic(1)).data.inst, sprintf('DISPLAY:ENABLE OFF'));
 %               fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENSE:FUNCTION "VOLT:DC"'));
%                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('FORMAT:DATA ASCII'));
%                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('FORMAT:ELEM READ'));%format statement for readings only
                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('SYSTEM:AZERO:STATE OFF'));%set up conditions for maximum measurement speed
                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENSE:VOLT:DC:AVERAGE:STATE OFF')); %turn off the filter for DCV
                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENSE:VOLT:DC:NPLC 0.01'));
                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENSE:VOLT:DC:RANGE 10'));
                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENSE:VOLT:DC:DIGITS 4'));
  %              fprintf(smdata.inst(ic(1)).data.inst,'trace:clear');
  %               fprintf(smdata.inst(ic(1)).data.inst, sprintf('INIT:CONT OFF'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRIG:SOUR IMM'));
   %              fprintf(smdata.inst(ic(1)).data.inst,sprintf('TRIG:TIM 0.1'));
      %          fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRIGGER:DELAY 0'));
%                  fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRAC:POIN %f',pts));
%                  fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRAC:FEED SENS'));
%                  fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRAC:FEED:CONT NEXT'));
   %             fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRIGGER:COUNT 1'));
                ...
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('INIT'));
                dmm_data = query(smdata.inst(ic(1)).data.inst, sprintf('fetch?'));
                %dmm_data=scanstr(smdata.inst(ic(1)).data.inst,',','%f');  
...
%                 dmm_data = fread(smdata.inst(ic(1)).data.inst, pts, 'float32');
                ...
                val=voltage %dmm_data
                toc
                 figure; plot(voltage);
            case 1 %write operation;  
                fprintf(smdata.inst(ic(1)).data.inst, sprintf(':DISPLAY:ENABLE ON'));
            otherwise
                error('Operation not supported');
        end
    case 7 %NoBuff.  Read rather than Trace
         switch ic(3); %Operation: 0 for read, 1 for write
            ...
            case 0 %read at max speed?
                ...
                tic
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('ABORT;*RST'));
                set(smdata.inst(ic(1)).data.inst, 'TIMEOUT',30);
%                 fprintf(smdata.inst(ic(1)).data.inst,'TRIG:SOUR EXT'); 
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('SENSE:FUNCTION "VOLT:DC"'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('FORMAT:DATA ASCII'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('FORMAT:ELEM READ'));%format statement for readings only
                ...
                fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRIGGER:SOURCE IMM'));
                fprintf(smdata.inst(ic(1)).data.inst,'trace:clear');
                fprintf(smdata.inst(ic(1)).data.inst,'trace:clear:auto on');
                fprintf(smdata.inst(ic(1)).data.inst, sprintf(':DISPLAY:ENABLE OFF'));
...
%                 end;
   ...
   %              fprintf(smdata.inst(ic(1)).data.inst,'trace:clear:auto on');
                 ...
                 fprintf(smdata.inst(ic(1)).data.inst, sprintf('INIT:CONT OFF'));
%                  fprintf(smdata.inst(ic(1)).data.inst,sprintf('TRIG:TIM 0'));
%                  fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRAC:POIN %f',pts));
%                  fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRAC:FEED SENS'));
%                  fprintf(smdata.inst(ic(1)).data.inst, sprintf('TRAC:FEED:CONT NEXT'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf(':TRIGGER:COUNT 1'));
                ...
                fprintf(smdata.inst(ic(1)).data.inst, sprintf(':TRIG:SOUR EXT'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf(':TRIGGER:DELAY 0'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf(':INIT'));
                fprintf(smdata.inst(ic(1)).data.inst, sprintf(':fetch?'))
       %         dmm_data = query(smdata.inst(ic(1)).data.inst, sprintf('fetch?'));
                dmm_data=scanstr(smdata.inst(ic(1)).data.inst,',','%f');  
%                 fclose(smdata.inst(ic(1)).data.inst);
%                 set(smdata.inst(ic(1)).data.inst,'InputBufferSize', pts*4);
