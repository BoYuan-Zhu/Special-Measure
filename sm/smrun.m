...
%            supposed to do.
        ...
        z = NaN(length(y),length(x));%zeros(length(y), length(x));
        z(:, :) = subsref(data{dc}, s);
        disph(i) = imagesc(x, y, z);
        %disph(i) = imagesc(x, y, permute(subsref(data{dc}, s), [ndim(dc)+(-1:0), 1:ndim(dc)-2]));
        
        set(gca, 'ydir', 'normal');
       % set(disph(i),'AlphaData',~isnan(z)); %HOHC 9.16.2013
        colorbar;
        if dc <= length(getch)
...
for i = 1:totpoints    
    ...
    % 12/2015: Added by Yuan
    % Now you can use trafofn to screen out points you want to skip
    % Simply return NaN for those points
    ...
    for j = loops(~isdummy(loops))
        ...
        for k = find([disp.loop] == j)
            ...
            if disp(k).dim == 2
                ...
                set(disph(k), 'cdata', z);
              %  set(disph(k),'AlphaData',~isnan(z)); %HOHC 9.16.2013
...
% FIX: save temporary data files for backup purpose since -append may fail
% and we lose all data
                [p,f,e] = fileparts(filename);
                if strcmp(p,'')
                    tempfile = sprintf('%s-temp%d%s~', f, count(j), e);
                else
                    tempfile = sprintf('%s/%s-temp%d%s~', p, f, count(j), e);
                end
    ...
    if get(figurenumber, 'CurrentCharacter') == char(27)
        ...
        %saveas(figurenumber,figstring,'fig'); %HOHC edit 
        %saveas(figurenumber,figstring,'pdf'); %HOHC edit
        set(figurenumber, 'CurrentCharacter', char(0));
        ...
        if isfield(scan,'exportfn')
            for k = 1:length(scan.exportfn)
                scan = scan.exportfn(k).fn(scan,scan.exportfn(k).args{:},'aborted');
            end
        end
...
saveas(figurenumber,figstring,'pdf');  
...
if isfield(scan,'exportfn')
   for k = 1:length(scan.exportfn)
      scan = scan.exportfn(k).fn(scan,scan.exportfn(k).args{:}); 
   end
end
