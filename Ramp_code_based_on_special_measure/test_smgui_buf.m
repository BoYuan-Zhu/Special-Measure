...
K2450_GPIB_2      = 19;  % K2450_2 GPIB address
...
%% -------------------- K2450 --------------------
try
    ind_k = smloadinst('k2450_2', [], 'ni', 0, K2450_GPIB_2);
    smopen(ind_k);
    smdata.inst(ind_k).name    = 'K2450';
    smdata.inst(ind_k).cntrlfn = @smcK2450_Ramp;

    % Source V (Vg) and Read I (Ig) and buffered Ig
    smaddchannel('K2450','Vg',     'V',     [-10, 10, Inf, 1]);      % source V
    smaddchannel('K2450','Ig',     'I',     [-Inf, Inf, Inf, 1e6]);  % read I
    smaddchannel('K2450','Ig-buf', 'Ig_buf_2');                         % buffered I
catch err
    fprintf(['*ERROR* problem with connecting to the Source | ' err.identifier ': ' err.message '\n']);
end
load smdata_empty;
