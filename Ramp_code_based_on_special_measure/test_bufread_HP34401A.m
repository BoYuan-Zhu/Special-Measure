...
GPIB_BOARD        = 'ni';
BOARD_NUM         = 0;
HP34401A_GPIB   = 22;   % SR830 GPIB address
...
%% -------------------- HP34401A --------------------
try
    ind_sr = smloadinst('sminst_HP34401A', [], GPIB_BOARD, BOARD_NUM, HP34401A_GPIB);
    smopen(ind_sr);
    smdata.inst(ind_sr).name    = 'HP34401A';
    smdata.inst(ind_sr).cntrlfn = @smcHP34401A_Ramp;

    % Buffered channels (Lock-in first in getchan to avoid time skew)
   
    smaddchannel('HP34401A','DATA','DATA');   % Phase trace

catch err
    fprintf(['*ERROR* HP34401A: ' err.identifier ': ' err.message '\n']);
end
