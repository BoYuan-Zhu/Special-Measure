        ...
        case 1 %V
            switch ic(3); %Operation: 0 for read, 1 for write
                ...
                case 0 %read
                    ...
                    txt = query(smdata.inst(ic(1)).data.inst, ':SOUR:VOLT:LEV:IMM:AMPL?');  % 或 ':VOLT?'
                    val = str2double(strtrim(txt));                 % -> double 标量
         ...
         case 5 % Ig-buf (K2400 buffered current helper)
            ...
            switch ic(3)
                case 3  % one-time configure & arm (prepare buffer & measurement path)
                    fprintf(inst, '*CLS');                    % 清错误/状态队列
                    
                  
                    fprintf(inst, ':TRIG:CLEar');             % 清触发模型  (Trigger subsystem)
                    fprintf(inst, ':ABORt');                  % 终止任何进行中的SDM/触发
                    fprintf(inst, ':TRAC:FEED:CONT NEVer');   % 暂停向缓冲写入（确保非active）
                    fprintf(inst, ':TRAC:CLEar');             % 现在清空缓冲不会被拒绝
                    
                    fprintf(inst, ':FORM:ELEM CURR');         % 只保留电流元素
                    fprintf(inst, ':SOUR:FUNC VOLT');         % 源电压-测电流
                    fprintf(inst, ':SOUR:VOLT:RANG 20');
                    fprintf(inst, ':SENS:FUNC "CURR"');
                    fprintf(inst, ':SENS:CURR:NPLC 1');
                    
                    fprintf(inst, ':TRAC:POIN 2500');         % 缓冲容量(1..2500)
                    fprintf(inst, ':TRAC:FEED SENS');         % 来自测量通道
                    fprintf(inst, ':TRAC:FEED:CONT NEXT');    % 每触发一次写下一格
                    fprintf(inst, ':TRIG:COUN 1');            % 每次 INIT 采1点
